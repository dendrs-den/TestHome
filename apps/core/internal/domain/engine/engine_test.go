...
func TestPrepareRoundCanSwitchTournamentContext(t *testing.T) {
	e := New()
	...
	_, err = e.Handle(commands.Command{
	...
	if err != nil {
		t.Fatalf("prepare round with switched context: %v", err)
	}

	st := e.State()
	if st.TournamentID != "t-2" {
		t.Fatalf("expected switched tournament id t-2, got %s", st.TournamentID)
	}
	if st.RoundID != "r-2" {
		t.Fatalf("expected switched round id r-2, got %s", st.RoundID)
	}
	if st.RoundState != RoundPrepared {
		t.Fatalf("expected prepared state, got %s", st.RoundState)
	}
}
