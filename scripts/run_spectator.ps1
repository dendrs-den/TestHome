...
$appDir = Join-Path $root "apps\spectator"
$url = "http://127.0.0.1:5190"

if (-not (Test-Path $appDir)) {
  throw "[spectator] folder not found: $appDir"
}
...
if ($Tauri) {
  ...
  Write-Host "Spectator started (tauri)."
  exit 0
}
...
if (Wait-HttpReady -TargetUrl $url) {
  ...
  Write-Host "Spectator started (dev) and opened in browser: $url"
} else {
  Write-Warning "Spectator dev server started, but browser was not opened automatically: $url"
}
